...
import { $, Observable, NotifyEventCallback, NotifyEvent, isObservable } from '../../reactor/mod.ts'

import { isValidHTMLAttribute } from './html-attributes.ts'
...
function observer(computed: Computed, callback: NotifyEventCallback<unknown>, name: string): unknown {
  ...
  return observe(observable, callback)

  // while (isObservable(computed)) {
  //   computed = computed() as Computed
  // }
  ...
  // while (typeof computed === 'function') {
  //   const value = computed()
  //   if (typeof value === 'function') {
  //     computed = value as Computed
  //   } else {
  //     break
  //   }
  // }
  ...
  // const observable = $(computed, name)
  // const value = observe(observable, callback)
...
function observeAttributes(tag: Tag, props: Props) {

  const tagName: string = typeof tag === 'function' ? tag.name : tag

  const patchAttribute = (prop: string) => {
    return ({ name, value }: NotifyEvent<unknown>) => {
      console.log('⚡️', 'computed-props', [name], 'changed!', value)
      props[prop] = value
  ...
  for (const prop in props) {
    ...
    props[prop] = observer(() => attribute(), patchAttribute(prop), `${tagName}.prop.${prop}`)

    // if (isValidHTMLAttribute(tagName, prop)) {
    //   props[prop] = observer(() => attribute(), patchAttribute(prop), `${tagName}.prop.${prop}`)
    // }
    ...
    */
    ...
    // props[prop] = observer(() => attribute(), patchAttribute(prop), `${typeof tag === 'function' ? tag.name : tag}.prop.${prop}`)
    // props[prop] = observer(() => attribute, patchAttribute(prop), `${typeof tag === 'function' ? tag.name : tag}.prop.${prop}`)
    // props[prop] = observer(attribute, patchAttribute(prop), `${typeof tag === 'function' ? tag.name : tag}.prop.${prop}`)
...
function observeChildren(tag: Tag, props: Props) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (typeOfNode === 'object' && (node as HResult).props) {
      observeChildren(tag, (node as HResult).props)
    } else if (typeOfNode === 'function') {
      children[i] = observer(() => (node as Computed)(), patchChildren(i), `${typeof tag === 'function' ? tag.name : tag}.children[${i}]`)
...
function jsx(tag: Tag | typeof jsx, props: Props): unknown {
  ...
  if (tag === jsx) {
    ...
    // observeAttributes(tag as Tag, props)
    // observeChildren(tag as Tag, props)
  }
